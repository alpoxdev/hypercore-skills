...
- 프로젝트 규칙인 `.hyper/git-worktree/<folder_name>` 아래 Git worktree를 만들고 관리합니다.
...
- `.hyper/git-worktree/<folder_name>` 규칙 표준화
...
- "이 브랜치를 `.hyper/git-worktree` 아래 워크트리로 만들어줘."
...
- 기본 경로 `.hyper/git-worktree/<folder_name>`에 생성한 뒤 후속 에이전트 작업 컨텍스트를 새 worktree로 이동합니다.
...
- 기본 루트: `<repo-root>/.hyper/git-worktree/`.
- 기본 경로: `<repo-root>/.hyper/git-worktree/<folder_name>`.
...
- 중첩 worktree 생성 전에 `.hyper/git-worktree/`가 ignore 또는 local exclude 되어 있는지 확인합니다.
...
7. 답변, 인자, 또는 기존 요청 맥락에서 `.hyper/git-worktree/<folder_name>`의 폴더명을 정합니다.
...
- [ ] `.hyper/git-worktree/`가 ignore/local exclude 처리되었습니다.
