...
If the fast helper fails or omits needed detail, fall back to `repo-discover.mjs`, `repo-status.mjs`, `git-commit.mjs`, and `git-push.mjs`.
...
| legacy helpers | `repo-discover.mjs` prunes generated trees, `repo-status.mjs` reuses captured status/stat output, and `git-push.mjs` independently preserves the same safe-push policy |
